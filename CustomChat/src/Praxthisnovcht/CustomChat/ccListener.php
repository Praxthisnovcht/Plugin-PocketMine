	...
	public function getFormattedMessage(Player $player, $message) {
		...
		if($this->pureperms) {
				...
				$levelName = $isMultiWorldEnabled ?  $player->getLevel()->getName() : null;
                 $format = str_replace("{PurePerms}", $this->pureperms->getUserDataMgr()->getGroup($player, $levelName)->getName(), $format);
            } else {
                return false;
                }
		if($this->killchat) {
		...
		if($this->massive) {
			...
			$format = str_replace ( "{Money}", "ERROR", $format);
                ...
                if ($c->facpro){
                    $format = str_replace("{Faction}", $c->getPlayerFaction($player->getName()), $format);
                }
                else{
                    $format = str_replace("{Faction}", "ERROR", $format);
                }
