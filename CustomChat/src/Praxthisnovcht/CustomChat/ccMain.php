...
class ccMain extends PluginBase implements CommandExecutor {
	...
	private $pureperms, $economyjob, $playerstats;
	...
	public function onLoad() {
                ...
                $this->getLogger()->info(TextFormat::YELLOW . "Loading CustomChat v_2.4.0 by Praxthisnovcht");
                $this->getLogger()->info(TextFormat::RED."* Fixed by HotFireyDeath");
                $this->getLogger()->info(TextFormat::RED."* Fixed by Cr7Princek");
	 ...
	 */
        public function getPlayerFaction($player_name){
//            return $this->getServer()->getPluginManager()->getPlugin($this->getConfig()->get("factionspro-instance"))->getPlayerFaction($player_name);
        }
	public function onEnable() {
		$this->enabled = true;
//                $this->facpro = false;
//                $this->facpro = $this->getServer()->getPluginManager()->isPluginEnabled($this->getServer()->getPluginManager()->getPlugin($this->getConfig()->get("factionspro-instance")));
		...
		if(!$this->getServer()->getPluginManager()->getPlugin("MassiveEconomy") == false) {
			...
			$this->log ( TextFormat::GREEN . "- CustomChat - Loaded With MassiveEconomy !" );
                ...
                if ($this->facpro){
                        $this->log ( TextFormat::BLUE . "- CustomChat - Loaded with FactionsPro!");
                }
